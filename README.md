...
секретом (`REFRESH_SECRET`) и хранится на сервере в `refresh_tokens.json` для валидации.
...
### Что сделано:
Реализован фронтенд на **React.js**. Роутинг организован через условный рендеринг компонентов на основе активной вкладки `(tabs)`.
...
устанавливается флаг `isActive: false.`. Заблокированный пользователь не может войти в систему.
